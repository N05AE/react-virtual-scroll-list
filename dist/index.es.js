...
`;
const PreRenderWrapper = styled.div`
  opacity: 0;
  display: inline-block;
  top: 0;
  left: 0;
...
const updateReactState = (thisObj, data, propName = null) => {
  if (!data || typeof data !== 'object' && !propName || !thisObj || !(thisObj instanceof Component)) {
    return;
  }
  if (!propName) {
...
class VirtualGrid extends Component {
  ...
  render() {
    ...
    return React.craeteElement(GridWrapper, {
      ref: this.wrapperRef,
      width: width,
      height: height,
      unit: unit,
      tabIndex: tabIndex,
      className: className
    }, !isInited && React.craeteElement(PreRenderWrapper, {
      ref: this.preRenderRef
    }, itemRenderer(preRenderData)), React.craeteElement(ContentContainer, _extends({
      ref: this.contentRef,
      unit: unit
    }, contentProps), renderDatas.map(render => React.craeteElement(ItemWrapper, {
      style: {
        top: render.top,
        left: render.left
      },
...
export { VirtualGrid, virtualGridProps };
